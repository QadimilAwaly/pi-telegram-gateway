...
import { getActiveTunnelInfo } from "./tunnel-manager";
...
async function main() {
  ...
  console.log(`☁️  Cloud Sync Status:  ${syncStatus}`);

  const tunnelInfo = getActiveTunnelInfo();
  const tunnelStatus = tunnelInfo.active
    ? `\x1b[32mActive (PID: ${tunnelInfo.pid})\x1b[0m ${tunnelInfo.url ? `\x1b[36m${tunnelInfo.url}\x1b[0m` : ""}`
    : "\x1b[90mInactive\x1b[0m";
